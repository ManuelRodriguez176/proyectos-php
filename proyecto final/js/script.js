...
function mostrarpuntaje(){
    window.location.href = "mostrar_puntua.php";
}
...
function restartGame() {
    scoreSpan.innerHTML = +score + ". Perdiste.";
    clearInterval(fruitGenerator);
    clearInterval(moving);
    body = [3, 2, 1];
    speed = 0.1;
...
function endGame() {
    // Aquí deberías detener cualquier intervalo de movimiento o generación de frutas si los tienes.

    // Obtener el puntaje actual
    var finalScore = document.querySelector('.score').innerHTML;

    // Enviar el puntaje al servidor usando AJAX
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "guardar_puntaje.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            console.log(xhr.responseText); // Puedes usar esto para depurar
        }
    };
